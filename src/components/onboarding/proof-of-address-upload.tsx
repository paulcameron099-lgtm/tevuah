...
type ProofOfAddressUploadProps = {
  userId: string;
  value: string | null;
...
export function ProofOfAddressUpload({
  ...
  async function handleFile(file: File) {
    ...
    const path =
      `${userId}/address/proof-of-address.${extension}`;

    const {
      error: uploadError,
    } = await supabase.storage
      .from("investor-verification")
      .upload(
        path,
        file,
        {
    ...
    <div className="rounded-[1.25rem] border border-forest-900/10 bg-ivory-50 p-5">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-xs leading-6 text-stone-500">
            Upload a recent utility bill, bank statement,
            government letter or other accepted address document.
          </p>
        </div>
      ...
      {value ? (
        <p className="mt-3 text-xs font-medium text-emerald-700">
          ✓ Proof of address uploaded securely
        </p>
      ) : null}
